...
const NAV: NavItem[] = [
  ...
  { href: '/firm/documents', label: 'Documents', icon: <IconFile /> },
];
  ...
  return (
    <div className="flex min-h-screen bg-bg">
      ...
      <aside className="w-[260px] bg-sidebar text-sidebar-fg flex flex-col py-5 sticky top-0 h-screen shrink-0">
        {/* Clockd brand */}
        <Link href="/firm" className="flex items-center gap-3 px-4 mb-4">
          <Logo height={24} />
        </Link>

        {/* Firm identity strip */}
        <div className="mx-3 mb-3 flex items-center gap-3 px-3 py-2.5 rounded-lg bg-white/5">
          <BHLogo size={36} />
          <div className="min-w-0 flex-1">
            <div className="text-sm font-semibold text-sidebar-fg truncate">{firm.name}</div>
            <div className="text-[11px] text-sidebar-muted truncate">{firm.location}</div>
          </div>
        </div>
        ...
        <div className="px-3 pt-3 border-t border-white/10 space-y-1">
          ...
          <Link
            ...
            <div className="relative shrink-0">
              <Avatar src={currentFirmUser.avatarUrl} name={currentFirmUser.name} initials={currentFirmUser.initials} size={36} bg="#FEF3C7" fg="#92400E" />
              <span className="absolute -bottom-0.5 -right-0.5 ring-2 ring-sidebar rounded">
                <BHBadge size={14} />
              </span>
            </div>
            <div className="min-w-0 flex-1">
