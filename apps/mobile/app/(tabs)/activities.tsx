// Activities — Sophia's time entries, scoped to her. Same layout as before
// (segmented filter + entry rows), but lives as a secondary tab now that
// Home owns the "morning glance" job.
...
export default function Activities() {
  ...
  const all = useMemo(() => {
    return [...submitted, ...seedEntries]
      .filter((e) => e.lawyerId === currentLawyer.id)
    ...
    <View style={[styles.root, { paddingTop: insets.top + space.md }]}>
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.sub}>Your time entries</Text>
        </View>
          ...
          return (
            <Pressable
              ...
              <View
                style={[
                  styles.entryIcon,
                  e.source === 'call' && { backgroundColor: colors.accentSoft },
                ]}>
                <Ionicons
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.entryMatter} numberOfLines={1}>
                  {matterDisplay(e.matterId)}
                </Text>
